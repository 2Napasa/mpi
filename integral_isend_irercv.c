/***********************************************************************
* FILENAME :        integral_isend_irecv.c
*
* DESCRIPTION :
...
*
* NOTES :
*       Output is handled by root process
* 
* AUTHOR :    Rauan Kelesbekov        START DATE :    21 Nov 20
...
int main(int argc, char** argv) {
    ...
    if (procrank != 0) { // all nonzero proc sending svoi int to rang0
        int dest = 0;
        MPI_Isend(&local_trapezoid,1,MPI_LONG_DOUBLE,dest,tag,MPI_COMM_WORLD, &send_request);
        MPI_Wait(&send_request, &status);
        // order does not matter isend irecv or irecv isend
    }
    else {
        ...
        for (int core=1; core < procnums; core++){
            MPI_Irecv(&local_trapezoid, 1, MPI_LONG_DOUBLE,MPI_ANY_SOURCE,tag,MPI_COMM_WORLD, &recv_request); 
