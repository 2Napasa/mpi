...
int main(int argc, char *argv[]) {
    ...
    while(1){
        ...
        if (rank == 2){
            for (int i = 0; i<0.2*N; i++){ // heat bot
                locgrid[0][i] = 1;
            }
        ...
        for (int i = 1; i < Ny-1; i++){
            for (int j = 1; j < N-1; j++){
                ...
                nextlocgrid[i][j] = ((locgrid[i+1][j]+locgrid[i-1][j])/dy/dy+(locgrid[i][j+1]+locgrid[i][j-1])/dx/dx+((j+xstart)*dx)*((j+xstart)*dx))/(2/dx/dx+2/dy/dy);
            }
        }
        ...
        if (rank ==0 ){
            for (int i = 1; i<size; i++){
                MPI_Recv(&maxDelta2,1,MPI_DOUBLE, MPI_ANY_SOURCE,1234,MPI_COMM_WORLD,&status);
        ...
        if (breakflag==1){
            ...
            char* a = "/Users/user/Documents/MATLAB/OUTnp4_dim2_"; 
            char* extension =".csv";
            ...
            for(int i = 1; i<Ny-2; i ++){
                for (int j = 1; j<N-2; j++){
                    fprintf(out,"%7.5f,h",locgrid[i][j]);
                }
                fprintf(out,"\n");
            }
            fclose(out);
